...
function AdminCrudPlaces() {
  ...
  const fetchPlaces = useCallback(async (searchTerm, page) => {
    try {
      setLoading(true);
      let res;
      if (searchTerm.trim() !== "") {
        res = await axios.get(`https://backend-mu-ten-26.vercel.app/places/search?q=${searchTerm}`);
        const result = Array.isArray(res.data) ? res.data : res.data.places || [];
        setPlaces(result);
        setNoResults(result.length === 0);
        setTotalPages(1);
      } else {
        res = await axios.get(`https://backend-mu-ten-26.vercel.app/admin/place?page=${page}`);
  ...
  const debouncedSearch = useCallback(
    debounce((searchValue) => fetchPlaces(searchValue, 1), 500),
    []
  );

  useEffect(() => {
    debouncedSearch(placeName);
  }, [placeName]);
