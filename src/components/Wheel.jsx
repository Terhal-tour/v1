...
const Wheel = () => {
  ...
  const fetchPlaces = async () => {
    try {
      const res = await axios.get(
        "https://backend-mu-ten-26.vercel.app/randomplaces/random"
      );
      setPlaces(res.data);
    } catch (err) {
  ...
  const spin = () => {
    ...
    const extraSpins = 6;
    const finalDeg =
      360 * extraSpins + (360 - index * degreesPerSlice) - degreesPerSlice / 2;
    ...
    >
      <div className="max-w-md w-full text-center mb-8">
        ...
        <p className="text-lg text-gray-600 mb-6">
          Take a chance and discover hidden gems across Egypt — your next
          adventure is just a spin away!
          <span className="block mt-2 text-md text-orange-500">
            Spin the wheel and let destiny guide you
          </span>
        </p>
      </div>
