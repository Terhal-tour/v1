  ...
  return (
    <nav className="bg-white sticky top-0 z-50 font-jakarta">
      ...
      <div className="container mx-auto flex items-center justify-between px-4 sm:px-6 py-4 navbar-container relative">
        ...
        <div
          ref={menuRef}
          className={`navbar-links fixed left-0 right-0 top-0 pt-24 pb-8 bg-white shadow-lg transition-all duration-300 z-50 flex-col items-center gap-6 text-lg ${menuOpen ? "open flex" : "hidden"} lg:static lg:flex lg:flex-row lg:justify-center lg:items-center lg:gap-10 lg:pt-0 lg:pb-0 lg:bg-transparent lg:shadow-none lg:text-base`}
          ...
          <NavLink
            ...
            onClick={e => {
              if (window.location.pathname === "/") {
                e.preventDefault();
                document.getElementById("topRated")?.scrollIntoView({ behavior: "smooth" });
              } else {
                e.preventDefault();
          ...
          <div className="flex flex-col gap-4 mt-6 w-full items-center lg:mt-0 lg:flex-row lg:gap-4 lg:w-auto lg:ml-8 lg:items-center">
            ...
            {token ? (
              <>
                ...
                <NavLink to={'/assistant'} className="flex items-center gap-3 bg-[var(--color-gold)] text-white font-bold py-3 px-6 rounded-full shadow-lg hover:bg-opacity-90 transform hover:-translate-y-1 transition-all duration-300 w-full max-w-xs text-center justify-center lg:w-auto lg:max-w-none"
                  onClick={() => setMenuOpen(false)}
                >
